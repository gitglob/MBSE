...
days_for_multiple_simulations = 1 # DAYS
sensor_static_values = ['True', 'False'] # True: 'Static', False: 'Dynamic'
sensor_period_values = [15*60, 60*60, 4*60*60] # Sensors' sampling period in seconds
sensor_distance_values = [8, 15, 30] # Sensors' distance in blocks
...
#%%
    ...
    if TIME_TO_RUN != 0:
        ...
        if RUN_ALL:
            for SENSOR_STATIC in sensor_static_values:
                for SENSOR_PERIOD in sensor_period_values:
                    for SENSOR_DISTANCE in sensor_distance_values:
                        
                        # print('SENSOR_STATIC',SENSOR_STATIC, 'SENSOR_PERIOD', SENSOR_PERIOD, 'SENSOR_DISTANCE',SENSOR_DISTANCE)
                        if SENSOR_STATIC:
                            placement = "static"
                        else:
                            placement = "dynamic"
